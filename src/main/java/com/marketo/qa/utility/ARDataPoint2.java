...
public class ARDataPoint2 {
	...
	public static void EventCampionAR(XSSFSheet AR_Data_Point2, XSSFWorkbook workbookoutput) throws Exception {
		...
		if (EventCampion == 0) {
			...
			try {
				ExcelStyling.pestImg(workbookoutput, AR_Data_Point2, 11, 36, passData.FetchScreenshot("Event"),0.6);
			...
			if (NurtureCampaigns == 0) {
				...
				try {
					ExcelStyling.pestImg(workbookoutput, AR_Data_Point2, 11, 46,
							passData.FetchScreenshot("Nurture campaigns"),0.6);
			...
			for (int i = 1; i < 3; i++)
				try {
					ExcelStyling.pestImg(workbookoutput, AR_Data_Point2, coll, 58,
							passData.FetchScreenshot("Segmentations" + i),0.6);
					coll = coll + 3;
			...
			if (Integration > 0) {
				...
				try {
					ExcelStyling.pestImg(workbookoutput, AR_Data_Point2, 10, 90,
							passData.FetchScreenshot("Integration"),0.6);
			...
			for (int i = 1; i <= WPCount; i++) {
				...
				try {
					ExcelStyling.pestImg(workbookoutput, AR_Data_Point2, coll, 98,
							passData.FetchScreenshot(passData.Exceldata(data, 2)),0.6);
					coll = coll + 3;
				...
				for (int k = 3; k <= ModelsCount; k++) {
					try {
						...
						ExcelStyling.pestImg(workbookoutput, AR_Data_Point2, coll, 102,
								passData.FetchScreenshotForApprovedModels(passData.Exceldata(data, k)),0.6);
						coll = coll + 7;
					} catch (Exception e) {
						// TODO: handle exception
					}
